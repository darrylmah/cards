...
struct TeamBuilderView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            let topH = geo.size.height * 0.33
            let hPad: CGFloat = 16
            let vPad: CGFloat = 12
            let formationCardW = (geo.size.width - hPad*2 - 8*3) / 4
            let formationCardH = formationCardW * 1.27
            let poolCols = [GridItem(.flexible(), spacing: 8),
                            GridItem(.flexible(), spacing: 8),
                            GridItem(.flexible(), spacing: 8)]
            let poolCardW = (geo.size.width - hPad*2 - 8*2) / 3
            let poolCardH = poolCardW * 1.27
            ...
            VStack(spacing: 12) {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 8) {
                            ForEach(0..<Board.maxSlots, id: \.self) { idx in
                                let card = board.slots[idx]
                                CardSlotView(card: card, index: idx, isHighlighted: draggingID != nil && draggingID != card?.id, isPlayerSide: true, onTap: {
                                    if let c = board.slots[idx] { board.slots[idx] = nil; pool.append(c) }
                                }, size: .init(width: formationCardW, height: formationCardH))
                                .onDrag {
                                    guard let c = board.slots[idx] else { return NSItemProvider() }
                                    draggingID = c.id
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    ScrollView(.vertical) {
                        LazyVGrid(columns: poolCols, spacing: 8) {
                            ForEach(visiblePool, id: \.id) { card in
                                ...
                                CardSlotView(
                                    ...
                                    size: .init(width: poolCardW, height: poolCardH)
                                )
                                .overlay(
